...
export default function UsersSection() {
  ...
  const handleDelete = () => {
    ...
    deleteUserMutation.mutate(target.id, {
      ...
      onError: (error) => {
        showAlert('error', extractErrorMessage(error), t('app.alert.error.delete_user') || 'Unable to delete user');
      },
    });
  };
    ...
    <>
      ...
      <Show when={usersQuery.isError}>
        <div class="mb-6 rounded-md border border-red-100 bg-red-50 p-4 text-sm text-red-700">
          {t('app.alert.error.load_users') || 'Unable to load users. Please refresh and try again.'}
        </div>
      </Show>
        ...
        fallback={
          <div class="flex h-64 items-center justify-center rounded-lg bg-white shadow">
            <div class="text-center">
              ...
              <p class="mt-4 text-sm text-gray-500">{t('app.loading.users') || 'Loading users…'}</p>
            </div>
          </div>
        }
      >
        ...
        <Show
          ...
          fallback={
            <div class="rounded-lg border border-dashed border-gray-300 bg-white p-12 text-center shadow-sm">
              <h3 class="text-lg font-semibold text-gray-900">{t('app.user.no_users') || 'No users yet'}</h3>
              <p class="mt-2 text-sm text-gray-500">{t('app.user.add_first_user') || 'Add your first user to start managing access.'}</p>
            ...
            <div class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                  <tr>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">{t('app.user.user') || 'User'}</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">{t('app.apps.password')}</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">{t('app.user.public_access') || 'Public access'}</th>
                      ...
                      <tr>
                        ...
                        <td class="px-6 py-4">
                          <span class={`inline-flex items-center rounded-full px-3 py-1 text-xs font-medium ${user.has_password ? 'bg-green-50 text-green-700' : 'bg-yellow-50 text-yellow-700'}`}>
                            {user.has_password ? (t('app.user.password_set') || 'Set') : (t('app.user.passwordless') || 'Passwordless')}
                          </span>
                        </td>
                        <td class="px-6 py-4">
                          <span class={`inline-flex items-center rounded-full px-3 py-1 text-xs font-medium ${user.public_front ? 'bg-blue-50 text-blue-700' : 'bg-gray-100 text-gray-600'}`}>
                            {user.public_front ? (t('app.user.allowed') || 'Allowed') : (t('app.user.restricted') || 'Restricted')}
                          </span>
                        </td>
                        <td class="max-w-xs px-6 py-4">
                          <Show when={user.autologin_url} fallback={<span class="text-xs text-gray-400">{t('app.user.disabled') || 'Disabled'}</span>}>
                            {(url) => (
                              <div class="space-y-1">
                                ...
                                <button
                                  type="button"
                                  class="text-xs font-medium text-gray-500 hover:text-gray-900 block"
                                  onClick={async () => {
                                    try {
                                      await navigator.clipboard.writeText(url());
                                      showAlert('info', t('app.alert.success.url_copied') || 'Autologin URL copied to clipboard', t('app.alert.copied') || 'Copied');
                                    } catch (error) {
                                      showAlert('error', t('app.alert.error.copy_failed') || 'Unable to copy URL. Copy it manually.', t('app.alert.error.clipboard') || 'Clipboard error');
                                    }
                                  }}
                                >
                                  {t('app.buttons.copy_link') || 'Copy link'}
                                </button>
                              </div>
                            )}
                          </Show>
      ...
      <ConfirmDialog
        ...
        title={t('app.user.delete_user') || 'Delete user'}
        message={t('app.user.delete_confirmation') || 'This action will permanently remove the user. This cannot be undone.'}
